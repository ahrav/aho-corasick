...
declare -ar controlled_environment=(
...
check_revision() {
    ...
    [[ -z "$ignored" ]] || {
        echo "$checkout: ignored files could affect the build:" >&2
        echo "$ignored" >&2
        exit 1
    }
}
...
(
    cd "$fork_checkout"
    run_controlled go test -count=1 ./...
...

{
    ...
    run_controlled go version
    run_controlled go env GOOS GOARCH GOARM64 GOAMD64 CGO_ENABLED
